...
module Fun =
  ...
  let show drawing =
    if (not lazyForm.IsValueCreated) || lazyForm.Value.IsDisposed then
      lazyForm <- createForm()
    lazyForm.Value.Visible <- true
    lazyForm.Value.Drawing <- drawing

  let private currentToken : ref<System.Threading.CancellationTokenSource option> = ref None
  ...
  let animate f =
    ...
    currentToken := Some cts
    Async.Start(loop 0.0, cts.Token)
    cts
